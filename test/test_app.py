...
from app import ReconciliadorEngine, app
...
class TestReconciliadorEngineAcademico:
    ...
    def test_processar_colunas_obrigatorias_template(self, tmp_path, csv_snapshot_arquivo):
        ...
        df_incompleto = pd.DataFrame({"id_cenario": [1], "nome_cenario": ["Teste"]})
    ...
    def test_processar_colunas_obrigatorias_snapshot(self, tmp_path, csv_template_arquivo):
        ...
        df_incompleto = pd.DataFrame({"id_origem": [1]})
    ...
    def test_processar_sanitizacao_nomes_coluna(self, tmp_path):
        ...
        df_template = pd.DataFrame(
            {"  ID_Cenario  ": [1], " Nome_Cenario ": ["Teste"], "ID_Roteiro_Esperado": [100]}
        )
        df_snapshot = pd.DataFrame(
            {" ID_Origem ": [1], "ID_Roteiro_Gerado": [100], "Valor_Evento": [1000]}
        )
    ...
    def test_processar_acuracia_zero_porcento(self, tmp_path):
        ...
        df_template = pd.DataFrame(
            {"id_cenario": [1, 2], "nome_cenario": ["A", "B"], "id_roteiro_esperado": [100, 200]}
        )
        df_snapshot = pd.DataFrame(
            {
                "id_origem": [1, 2],
    ...
    def test_processar_roteiros_como_float(self, tmp_path):
        ...
        df_template = pd.DataFrame(
            {"id_cenario": [1], "nome_cenario": ["Teste"], "id_roteiro_esperado": [100.0]}
        )
        df_snapshot = pd.DataFrame(
            {
                "id_origem": [1],
...
class TestFlaskAppAcademico:
    ...
    def test_index_post_um_arquivo_faltando(self, client, tmp_path):
        ...
        pd.DataFrame(
            {"id_cenario": [1], "nome_cenario": ["A"], "id_roteiro_esperado": [100]}
    ...
    def test_index_post_arquivo_formato_invalido(self, client, tmp_path):
        ...
        pd.DataFrame({"id_origem": [1], "id_roteiro_gerado": [100]}).to_csv(csv_file, index=False)
        ...
        df_template = pd.DataFrame(
            {
                ...
                "nome_cenario": [
                    "Processamento Folha 1",
                    "Processamento Folha 2",
                    "Processamento Folha 3",
                    "Folha Especial 1",
                    "Folha Especial 2",
                    "Folha Bônus 1",
                    "Processamento Folha 4",
                    "Processamento Folha 5",
                    "Folha Especial 3",
                    "Folha Bônus 2",
                ],
