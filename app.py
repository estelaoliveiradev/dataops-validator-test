...
class ReconciliadorEngine:
    ...
    def ler_arquivo(file):
        ...
        try:
            ...
            raise ValueError(f"Erro ao processar o arquivo {filename}: {e!s}")
    ...
    def processar(file_template_expectativa, file_snapshot_processado):
        ...
        if not cols_template.issubset(df_template.columns):
            raise ValueError(
                f"TemplateExpectativa incompleto. Colunas necessárias: {cols_template}"
            )
        ...
        return {
            ...
            "metricas": {"acuracia": round(acuracia, 2), "tempo": tempo_total, "total": total},
    ...
    if request.method == "POST":
        ...
        if file_template and file_snapshot:
            try:
                ...
                fig.update_layout(margin={"t": 0, "b": 0, "l": 0, "r": 0}, showlegend=True)
        ...
        df = pd.DataFrame(
            {
                "id_cenario": [1, 2],
                "nome_cenario": [
                    "Exemplo: Roteiro 1 - Folha Padrão",
                    "Exemplo: Roteiro 2 - Folha Especial",
                ],
